...
const PipelineManager: React.FC<PipelineManagerProps> = ({ onSelectPipeline }) => {
    ...
    const handleDeleteSelected = async () => {
        try {
            setIsLoading(true);
            const deletePromises = selectedPipelines.map((id) =>
            ...
            if (results.every((success) => success)) {
                ...
                setExpandedPipelines(
                    expandedPipelines.filter((pid) => !selectedPipelines.includes(pid))
                );
            } else {
                        ...
                        <thead className="bg-gray-50 dark:bg-gray-700">
                            <tr>
                              ...
                              text-blue-600 focus:ring-blue-500"
                                ...
                                <th className="w-10 px-2 py-3">{/* Expand column */}</th>
                                <th
                                                    ...
                                                    <ol className="list-decimal list-inside pl-2 space-y-1.5">
                                                        {pipeline.nodes.length > 0 ? (
                                                            pipeline.nodes.map((node, index) => (
                                                                <li
                                                                    key={node.id}
                                                                    className="py-1 px-2 rounded bg-gray-100 dark:bg-gray-600"
                                                                >
