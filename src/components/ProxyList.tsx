...
const ProxyList: React.FC<ProxyListProps> = ({ isDarkMode }) => {
    ...
    const [proxyFilter, setProxyFilter] = useState<'all' | 'mapped' | 'unmapped'>('all');
    ...
    // Apply filtering and sorting to the proxy list
    const filteredAndSortedProxies = useMemo(() => {
        let filtered = proxies;

        // Apply mapping filter
        if (proxyFilter !== 'all') {
            filtered = proxies.filter((proxy) => {
                const isMapped = proxy.mappedEmailId !== null;
                return proxyFilter === 'mapped' ? isMapped : !isMapped;
            });
        }
        ...
        return filtered;
    }, [proxies, proxyFilter, sortField, sortDirection]);

    // Get current items for display after filtering and sorting
    const currentItems = filteredAndSortedProxies.slice(startIndex, endIndex);
    ...
    const fetchProxies = useCallback(async () => {
        try {
            ...
            if (fetchedProxies && Array.isArray(fetchedProxies)) {
                setProxies(
                    fetchedProxies.map((proxy: Proxy) => ({
                        ...proxy,
                        selected: false,
                    }))
                );
    ...
    const handleDeleteSelected = async () => {
        ...
        if (
            !window.confirm(
                `Are you sure you want to delete ${selectedIds.length} selected proxies?`
            )
        ) {
            return;
        }
    ...
    const handleSelectItem = (id: string, checked: boolean) => {
        setProxies((prev) =>
            prev.map((proxy) => (proxy.id === id ? { ...proxy, selected: checked } : proxy))
        );
    ...
    return (
        <div className="h-full flex flex-col">
            {/* Proxy List Section */}
            <div
                className={`p-4 sm:p-6 rounded-lg ${isDarkMode ? 'bg-gray-800/50' : 'bg-white/50'}`}
            >
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h3 className="text-xl font-semibold dark:text-white">Proxy List</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            Total: {filteredAndSortedProxies.length} proxies
                        </p>
                    </div>
                    <div className="flex space-x-2">
                        <select
                            value={proxyFilter}
                            onChange={(e) =>
                                setProxyFilter(e.target.value as 'all' | 'mapped' | 'unmapped')
                            }
                        ...
                        >
                            ...
                            <option value="mapped">Mapped Proxies</option>
                            <option value="unmapped">Unmapped Proxies</option>
                        </select>
                        <button
                        ...
                        <table className="w-full border-collapse table-fixed">
                            <thead className="sticky top-0 z-10">
                                ...
                                >
                                    ...
                                    <th className="p-2 sm:p-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                                        Host
                                    </th>
                                    <th className="p-2 sm:p-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                                        Port
                                    ...
                                    <th className="w-16 p-2 sm:p-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                                        Actions
                                    </th>
                                </tr>
                            </thead>
